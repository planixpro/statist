...
if ($period === 'all') {
    ...
    $df = date('Y-m-d', strtotime('-' . $periodMap[$period]['days'] . ' days'));
    $dt = date('Y-m-d');
...
if (!function_exists('pageUrl')) {
    ...
    function tabUrl(string $tab): string {
        $params = $_GET; $params['tab'] = $tab;
        unset($params['pg'], $params['pgb']);
        return '?' . http_build_query($params);
    }
}
...
if ($type === 'countries') {
    $total = (int)qv($pdo,
        "SELECT COUNT(DISTINCT country_code) FROM sessions
    ...
    $rows = q($pdo,
        "SELECT country, country_code, COUNT(DISTINCT session_id) as cnt
         FROM sessions WHERE site_id=? AND is_bot=0 AND is_valid=1 AND country IS NOT NULL$dateCond
         GROUP BY country, country_code ORDER BY cnt DESC LIMIT $perPage OFFSET $offset",
        [$activeSite]);

} elseif ($type === 'cities') {
    $total = (int)qv($pdo,
        "SELECT COUNT(DISTINCT city) FROM sessions
    ...
    $rows = q($pdo,
        "SELECT city, country, country_code, COUNT(DISTINCT session_id) as cnt
         FROM sessions WHERE site_id=? AND is_bot=0 AND is_valid=1 AND city IS NOT NULL$dateCond
         GROUP BY city, country, country_code ORDER BY cnt DESC LIMIT $perPage OFFSET $offset",
        [$activeSite]);

} elseif ($type === 'os' || $type === 'browsers') {
    $parseType = ($type === 'browsers') ? 'browser' : 'os';
    $uaRows = q($pdo,
        "SELECT user_agent, COUNT(*) as cnt FROM sessions
    ...
    $total = (int)qv($pdo,
        "SELECT COUNT(DISTINCT screen) FROM sessions
         WHERE site_id=? AND is_bot=0 AND is_valid=1 AND screen IS NOT NULL$dateCond",
        [$activeSite]);
    $rows = q($pdo,
        "SELECT screen, COUNT(*) as cnt FROM sessions
         WHERE site_id=? AND is_bot=0 AND is_valid=1 AND screen IS NOT NULL$dateCond
         GROUP BY screen ORDER BY cnt DESC LIMIT $perPage OFFSET $offset",
        [$activeSite]);
    ...
    $total = (int)qv($pdo,
        "SELECT COUNT(DISTINCT e.path)
         ...
         WHERE e.site_id=? AND e.event_type='page_view' AND s.is_bot=0 AND s.is_valid=1$dateCond2",
        [$activeSite]);
    $rows = q($pdo,
        "SELECT e.path, COUNT(*) as cnt
         FROM events e JOIN sessions s ON s.session_id=e.session_id AND s.site_id=e.site_id
         WHERE e.site_id=? AND e.event_type='page_view' AND s.is_bot=0 AND s.is_valid=1$dateCond2
         GROUP BY e.path ORDER BY cnt DESC LIMIT $perPage OFFSET $offset",
        [$activeSite]);

} elseif ($type === 'referrers') {
    $total = (int)qv($pdo,
        "SELECT COUNT(DISTINCT referrer) FROM sessions
         WHERE site_id=? AND is_bot=0 AND is_valid=1 AND referrer IS NOT NULL AND referrer != ''$dateCond",
        [$activeSite]);
    $rows = q($pdo,
        "SELECT referrer, COUNT(*) as cnt FROM sessions
         WHERE site_id=? AND is_bot=0 AND is_valid=1 AND referrer IS NOT NULL AND referrer != ''$dateCond
         GROUP BY referrer ORDER BY cnt DESC LIMIT $perPage OFFSET $offset",
        [$activeSite]);
}

if ($rows) $maxCount = max(array_column($rows, 'cnt'));
