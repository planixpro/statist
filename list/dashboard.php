...
if ($siteIds && !in_array($activeSite, array_map('intval', $siteIds), true)) {
...
$totalVisitors = (int)qv(
    $pdo,
    "SELECT COUNT(DISTINCT session_id)
     FROM sessions
     WHERE site_id=? AND is_bot=0 AND is_valid=1$dateWhereSess$cWhere",
    [...$pBase, ...$dateParamsSess, ...$cParam]
);

$totalPageviews = (int)qv(
    $pdo,
    "SELECT COUNT(*)
     FROM events e
...
$totalBots = (int)qv(
    ...
    "SELECT COUNT(*)
     FROM sessions
     WHERE site_id=? AND is_bot=1$dateWhereSess$cWhere",
    [...$pBase, ...$dateParamsSess, ...$cParam]
);

function buildChartSeries(array $rawData, ?string $dateFrom, ?string $dateTo): array
{
    if ($dateFrom !== null) {
...
$topPages = q(
    ...
    "SELECT
        e.path,
        MAX(NULLIF(TRIM(e.title), '')) as title,
        COUNT(*) as cnt
     FROM events e
     JOIN sessions s ON s.session_id=e.session_id AND s.site_id=e.site_id
     WHERE e.site_id=? AND e.event_type='page_view' AND s.is_bot=0 AND s.is_valid=1$dateWhereEvt$cWhereJ
...
$totalAllSess = (int)qv(
    ...
    "SELECT COUNT(*)
     FROM sessions
     WHERE site_id=? AND (is_bot=1 OR is_valid=1)$dateWhereSess$cWhere",
    [...$pBase, ...$dateParamsSess, ...$cParam]
);
